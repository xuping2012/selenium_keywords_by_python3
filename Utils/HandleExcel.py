...
class HandleExcel(object):
    ...
    def __init__(self, file_path=None):
        ...
        self.colorDict = {"black": 0, "white": 1, "red": 2, "green": 3, "bule": 4, "yellow": 5}
        self.pattern.pattern_fore_colour = self.colorDict.get('green')
    ...
    def set_wsheet_index_or_name(self, sheet):
        """设置将要写入内容的sheet对象，根据sheet索引或sheet名称"""
    ...
    def set_rsheet_index_or_name(self, sheet=0):
        """设置当前要读取的sheet对象"""
        self.wsh = self.wb.get_sheet(sheet)
    ...
    def write_cell_result(self, row_no, col_no, content, style=None):
        """
        xlwd写入结果文件
        row_no:行号, 从1开始的
        col_no:列号，从1开始的
        ...
        """
        self.wsh.write(row_no, col_no, content, style=self.style)
        self.save_workbook()
...
if __name__ == '__main__':
    ...
    handle_excel = HandleExcel(file_path=excel_path + "/关键字驱动测试用例.xlsx")
    # for j in range(len(handle_excel.get_sheets())):
    #     case_lines = handle_excel.get_lines(j)
    #     for i in range(1, case_lines):
    #         case_id = handle_excel.get_cell(i, 0, j)
    #         print(case_id)

    # test write same data in excel
    for sheet_name in range(len(handle_excel.get_all_sheets)):
        handle_excel.set_rsheet_index_or_name(sheet_name)
    ...
    for sheet_index in range(len(handle_excel.get_all_sheets)):
        handle_excel.set_rsheet_index_or_name(sheet_index)
        case_lines = handle_excel.get_max_rows
        for i in range(1, case_lines):
